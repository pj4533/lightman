...
program.version('0.0.6')
  ...
  .option('-an, --appname [app name]', 'Use App Name')
  .option('-sk, --skunumber [sku number]', 'Use SKU Number')
  .option('-vn, --versionnumber [version number]', 'Use Version Number')
  .option('-ds, --desc [description]', 'Use Description')
...
program.on('--help', function(){
  ...
  console.log('    itunes:addapp - adds new app to iTunes Connect');
  console.log('    itunes:updateapp - update exisiting app');
  console.log('    fb:getapps - gets a list of all available apps in Facebook.');
  console.log('');
});
...
} else if (program.args[0] == "itunes:updateapp") {
  itunes.updateApp(program.appname, program.versionnumber, program.desc);
} else if (program.args[0] == "itunes:addapp") {
  itunes.addNewApp(program.appname, program.skunumber);
} else if (program.args[0] == 'fb:getapps'){
  facebook.getApps();
}